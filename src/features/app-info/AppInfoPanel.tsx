  ...
  return (
    <details className="app-info-panel">
      ...
      <div className="app-info-panel__body">
        ...
        <h4>使い方</h4>
        <ol>
          <li>最初の画面で「歩行解析」「ステップ動作解析」のどちらを行うか選択する（後から切り替え可能）。</li>
          <li>動画（MP4・推奨H.264）とCSVを選択する。</li>
          <li>CSVの列割当を確認し（自動認識されない場合は手動で選択）、大腿長・下腿長・骨盤幅を入力する。</li>
          <li>
            歩行解析：動画を再生・コマ送りしながらRt_IC・Lt_IC・Rt_Off・Lt_Offを該当フレームで登録すると、歩行相時間・ケイデンスが算出される。歩幅は、歩幅波形をドラッグで範囲選択すると、その範囲内の極大・極小点から自動検出される。
          </li>
          <li>ステップ動作解析：ステップ動作（片脚での踏み出し）を登録すると、ステップごとの歩幅が算出される。</li>
